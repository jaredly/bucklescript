...
(** {1 Belt.List}
    ...
    @doc length, size, head, tail, add, get, make, makeBy

    {2 Manipulation}

    @doc shuffle, reduce, forEach, map, mapReverse, mapWithIndex, reduceReverse

    {2 Checking}

    @doc every, some, has

    {2 Working with "associative lists"}

    These can be used in place of a Map or HashMap if you know the length will remain fairly small.
    Note that the time that these functions take grows linearly with the size of the list. For some algorithms, this produces a performance bottleneck.

    @doc getAssoc, hasAssoc, setAssoc, removeAssoc

    {2 Converting}

    @doc ofArray, fromArray, toArray
...
(**
   @example {[
     let () = assert(head [] = None );;
     let () = assert(head [1;2;3] = Some 1 );;
   ]}
*)
...
(** [get xs n]
    ...
    @example {[
      let () = assert(get [0;3;32] 2 = Some 2)
      let () = assert(get [0;3;32] 3 = None)
    ]}
 *)
...
(** [makeBy n f]
    ...
    @example {[
      let () = assert(makeBy 5 (fun i -> i) = [0;1;2;3;4])
    ]}
*)
...
(** [drop xs n]
    ...
    @example {[
      let () = assert(drop [1;2;3] 2 = Some [3])
      let () = assert(drop [1;2;3] 3 = Some [])
      let () = assert(drop [1;2;3] 4 = None)
    ]}
*)
...
(** [take xs n]
    ...
    @example {[
      let () = assert(take [1;2;3] 1 = Some [1])
      let () = assert(take [1;2;3] 2 = Some [1;2])
      let () = assert(take [1;2;3] 4 = None)
    ]}
*)
...
(**
   ...
   @example{[
     let () = assert(splitAt [0;1;2;3;4] 2 = Some ([0;1], [2;3;4]))
   ]}
*)
