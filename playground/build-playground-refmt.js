...
// NOTE(jared): I'm not sure if we actually need to custom ocaml build...
...
const jsComp = path.join(__dirname, '..', 'jscomp')
...
//   env: {
//       ...process.env,
//       PATH: `${process.env.PATH}:${customOcaml}`,
//   },
  encoding: "utf8",
  stdio: [0, 1, 2],
  shell: true
...
function getOutput(cmd) {
  console.log(`>>>>>> running command: ${cmd}`);
  const result = p.execSync(cmd, Object.assign({}, { encoding: 'utf8', stdio: "pipe" }));
  console.log(`<<<<<<`);
  return result;
}

if (process.env.BS_PLAYGROUND == null) {
  process.env.BS_PLAYGROUND = path.join(__dirname, "output");
}
...
if (!fs.existsSync(jsComp + '/bin/bspack.exe')) {
    p.execSync('make -C bin', {cwd: jsComp})
}
...
function bspack() {
  ...
  let ocamlMigrateParseTreeSourceDir = getOutput(
    `esy echo "#{@opam/ocaml-migrate-parsetree.target_dir '/' '_build/default/src'}"`
  ).trim();

  const ompDir = path.join(__dirname, 'omp')
  if (!fs.existsSync(ompDir)) {
    fs.mkdirSync(ompDir);
  }
  fs.readdirSync(ocamlMigrateParseTreeSourceDir).forEach(name => {
    if (name.endsWith(".pp.ml") || name.endsWith(".pp.mli")) {
      fs.writeFileSync(
        path.join(ompDir, name.replace(".pp.", ".")),
        fs.readFileSync(path.join(ocamlMigrateParseTreeSourceDir, name))
      );
    }
  });
  let ocamlMigrateParseTreeTargetDir = ompDir;
  let reasonTargetDir = getOutput(
    `esy echo "#{@opam/reason.lib / 'reason'}"`
  ).trim();
  ...
  -I "${reasonTargetDir}" \
  -I "${reasonTargetDir}/easy_format" \
  -I "${reasonTargetDir}/cmdliner" \
...
function build() {
  ...
  var includes = [`stdlib`, `runtime`, `others`]
    .map(x => path.join(jsComp, x))
    .map(x => `-I ${x}`)
    .join(` `);
  ...
  console.log(`🎉🎉 Compiler created! Located at ${playground}/playground-refmt.js

    [The warning above about "SomeOCaml interface files were not found" is expected, and isn't a problem].
    `);
}

// These two files are duplicated from the reason sources, and bspack can't handle there being multiples
// of any files. So we have to "hide" these temporarily.
e(
